...
const ITEM_COLS = `asset_id, file_name, taken_at, size_bytes, width, height, score, decision`;
  ...
  const rows = db
    .prepare(
      `SELECT ${ITEM_COLS} FROM asset_state
       ...
       ORDER BY (decision IS NOT NULL) ASC, taken_at DESC
       LIMIT ?`
    )
...
export function getQueues(db: Db): QueuesSummary {
  const shot = db
    .prepare(
      `SELECT COUNT(*) c, COALESCE(SUM(size_bytes), 0) b FROM asset_state
       WHERE category = 'screenshot' AND (decision IS NULL OR decision = 'later') AND is_favorite = 0`
    )
    .get() as { c: number; b: number };

  // アクション可能な重複グループ = 未判断メンバーが2枚以上残っているもの。
  // 解放見込み = ベスト以外の未判断メンバーの合計サイズ。
  const dupRows = db
    .prepare(
      `SELECT g.id,
  ...
  return {
    screenshot: { count: shot.c, bytes: shot.b },
    duplicates: {
      groups: dupRows.length,
      bytes: dupRows.reduce((s, r) => s + (r.potential || 0), 0),
    },
...
export function getGroups(db: Db, kind: string): GroupView[] {
  const groups = db.prepare(`SELECT id, kind, best_asset_id FROM groups WHERE kind = ?`).all(kind) as {
    id: number;
    kind: string;
...
export function getStats(db: Db): Stats {
  ...
  const reviewed = db
    ...
    .get() as { c: number };
  return { freedBytes: freed.b, trashedCount: freed.c, reviewedCount: reviewed.c };
}
