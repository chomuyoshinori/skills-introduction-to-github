...
import { runScanOnce } from './services/scan';
...
if (count.c === 0) {
  console.log('初回スキャンを開始します…');
  runScanOnce(db, immich)
    .then((r) => console.log(`スキャン完了: ${r.scanned}枚 / スクショ候補 ${r.screenshots}枚 / 重複 ${r.duplicateGroups}組`))
