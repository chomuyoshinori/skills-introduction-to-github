...
| データ | 置き場所 | 補足 |
|---|---|---|
| 写真・動画の**原本**+サムネイル・再生用動画 | **外付けSSD 2TB**(`.env` の `UPLOAD_LOCATION` で指定) | 容量を食う本体はすべてここ。内蔵に写真は溜まらない |
| Immich のデータベース(PostgreSQL) | Mac mini **内蔵**ストレージ(`DB_DATA_LOCATION` は既定のまま) | 数GB程度。**外付けに置かない**こと — 接続断でDB破損のリスクがあり、Immich公式も外付け/NAS上のDBを非推奨 |
| Sift の判断履歴(`sift.db`) | Mac mini 内蔵(`photo-app/sift/server/`) | 数MB。写真は複製しない設計(SPEC §4) |
| バックアップ先(Time Machine) | **別の**外付けHDD | SSDのデータ量以上の容量を用意。内蔵(DB含む)とSSDの両方を対象にする |

つまり「写真の実体は外付けSSD、小さな管理データだけ内蔵」が正しい配置。
`UPLOAD_LOCATION` をSSDに向けた時点で仕様どおりになる。
